...
export default class StitchesPuzzle extends Puzzle<SquareGrid> {
  ...
  colHints: number[];
  rowHints: number[];
  areaEdges: Record<string, GridEdge[]>;
  constructor({
    stitch_count,
    areas,
    sums,
    ...
    for (const id of Object.keys(this.areaEdges)) {
      if (this.areaEdges[id].length == 0) delete this.areaEdges[id];
      else this.addConstraint(SUM_EQUALS, this.areaEdges[id], stitch_count);
    }
