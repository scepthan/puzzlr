...
export default class PuzzleGrid {
  ...
  #height;
  #finalized = false;
  #edgeMap = new Map<string, GridEdge>();
  onNewCell?: (cell: GridCell) => void;
  ...
  addEdge(fromVert: GridVertex, toVert: GridVertex, vpos?: any, noCell: boolean = false) {
    ...
    if (noCell) return newEdge;

    // If the vertices were previously connected and the new edge does not
    // split an existing cell, we must be creating a new cell
    if (this.#checkNewCell(fromVert, toVert)) return newEdge;
    if (this.#checkNewCell(toVert, fromVert)) return newEdge;
   ...
   */
  getEdge(fromVert: GridVertex | number, toVert: GridVertex | number) {
   ...
   */
  #checkNewCell(fromVert: GridVertex, toVert: GridVertex) {
  ...
  #addCell(verts: GridVertex[]) {
    ...
    for (let i = 0; i < verts.length; i++) {
      const currVert = verts[i],
        nextVert = verts[i + 1] || verts[0];
      ...
      if (currEdge.fromVert == currVert) currEdge.rightCell = newCell;
      else currEdge.leftCell = newCell;
    }

    if (typeof this.onNewCell == "function") this.onNewCell(newCell);
  }
   ...
   */
  angleBetweenEdges(edge1: GridEdge, edge2: GridEdge) {
    ...
    if (edge1.fromVert == edge2.fromVert) {
      ...
      pivot = edge1.toVert;
      vert2 = edge2.fromVert;
    } else throw new Error("Edges must share a vertex");
